...
- [PWB-001] [HIGH] [ROBUSTNESS] Preserve actionable OpenSSH authentication/configuration diagnostics when either control-master startup path fails. Evidence: `TestControlMasterReportsSSHStartupFailure` reproduced the missing `Permission denied (publickey).` detail; the pre-existing shared-master edit exposed the same root cause.
